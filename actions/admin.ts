...
export async function addStaff(formData: FormData) {
  ...
  const { error } = await supabase.from("staff").insert({ ...parsed.data, restaurant_id });
...
export async function addInventoryItem(formData: FormData) {
  ...
  const supabase = await createClient();
  const restaurant_id = await getCurrentRestaurantId();
  const { error } = await supabase.from("inventory_items").insert({ ...parsed.data, restaurant_id });
  if (error) return { error: error.message };
...
export async function updateRestaurantSettings(formData: FormData) {
  ...
  const { error } = await supabase.from("restaurants").update({ name: parsed.data.name, phone: parsed.data.phone, address: parsed.data.address, currency: parsed.data.currency }).eq("id", restaurantId);
  if (error) return { error: error.message };
