...
export default async function ReportsPage({ searchParams }: { searchParams?: { start?: string; end?: string; preset?: string } }) {
  ...
  const [{ data: sales }, { data: purchases }, { data: advances }, { data: expenses }, { data: usage }, { data: lowStock }, { data: items }] = await Promise.all([
    ...
    supabase.from("staff_advances").select("amount").eq("restaurant_id", restaurantId).gte("advance_date", start).lte("advance_date", end),
    supabase.from("daily_expenses").select("amount").eq("restaurant_id", restaurantId).gte("expense_date", start).lte("expense_date", end),
    supabase.from("inventory_movements").select("total_cost").eq("restaurant_id", restaurantId).eq("movement_type", "usage").gte("created_at", startTs).lte("created_at", endTs),
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="card p-4"><p className="text-sm text-muted">Advances Total</p><p className="mt-2 text-2xl text-accent">{formatCurrency(advancesTotal)}</p></div>
        <div className="card p-4"><p className="text-sm text-muted">Daily Expenses Total</p><p className="mt-2 text-2xl text-accent">{formatCurrency(expensesTotal)}</p></div>
        <div className="card p-4"><p className="text-sm text-muted">Usage Cost Total</p><p className="mt-2 text-2xl text-accent">{formatCurrency(usageCostTotal)}</p></div>
        <div className="card p-4"><p className="text-sm text-muted">Daily Cash Result</p><p className="mt-2 text-2xl text-accent">{formatCurrency(dailyCashResult)}</p></div>
        <div className="card p-4"><p className="text-sm text-muted">Daily Gross from Usage</p><p className="mt-2 text-2xl text-accent">{formatCurrency(dailyGrossFromUsage)}</p></div>
